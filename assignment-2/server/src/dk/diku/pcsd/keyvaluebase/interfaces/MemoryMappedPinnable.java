...
/**
 * This class was adapted to make its use easier. Unpin is not used. All data is
 * kept in memory only until it is flushed. Overlapping regions may be written
 * be kept in pinned_regions. A LinkedHashMap is used to preserve the order of
 * updates, so that newer changes will override older ones and there are no
 * conflict.
 * 
 */
public class MemoryMappedPinnable extends MemoryMappedFile {
	...
	@Override
	public void get(byte[] dst, long offset) throws IndexOutOfBoundsException,
	 ...
	 * @author PCSD-DIKU
	 * 
	 *         This class defines a region in main memory that can be pinned. It
	 *         offers basic methods to get its characteristics. It works with a
	 *         long byte array as a back up for the pinned data.
	 */
	public static class PinnedRegion implements Comparable<PinnedRegion> {
		...
		/**
		 * Get the position where this region starts.
		 * 
		...
		/**
		 * Get the position where this region ends.
		 * 
		...
		/**
		 * Compare this region's starting point with another given region. It
		 * returns -1, 0 or 1 depending on if this region starts before, at the
		 * same address, or after the given region.
		 * 
		 * @param region
		 *            the region with which to Returns compare this region
		 * 
		 * @return the position relative to the regions starting position
		 */
		@Override
		...
		/**
		 * Check if this region overlaps at some point with a given region
		 * 
		 * @param region
		 *            the region with which to check this region for overlap
		 * 
		 * @return whether the given region overlaps this region at some address
		 */
		public boolean overlaps(PinnedRegion region) {
			return ((this.startPosition >= region.startPosition && this.startPosition < region
					.getEndPosition()) || (region.startPosition >= this.startPosition && region.startPosition < this
					.getEndPosition()));
		}
